...
AND (0 <= ANY ( 
    ...
    OR -100000 = ALL ( 
        select unnest(array_agg(( 
            select sum(value) from vote where source_id = answer_id and source_type = 'answer'))) from answer where question_id = q.question_id) IS NULL)
    or ( 
        select count(answer_id) from answer where question_id = q.question_id) = 0
    and array['react'] = ( 
        select array_agg(tag_name) from question_tag where question_id = q.question_id)
